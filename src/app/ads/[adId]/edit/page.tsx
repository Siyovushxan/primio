...
export default function EditAdPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload first: the server moderates the stored image, not bytes sent by the browser
      if (newImageFile) {
      ...
      const updateRes = await fetch(`/api/ads/${adId}/update`, {
        ...
        body: JSON.stringify({
          ...
          imageURL: finalImageURL,
          moderationId: modData.moderationId,
        }),
      });
      if (!updateRes.ok) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "system-ui,sans-serif" }}>
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 80, background: "rgba(11,11,15,.94)", backdropFilter: "blur(18px)", borderBottom: "1px solid var(--border)" }}>
        <div style={{ padding: "11px 26px", display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
            ...
            <Link href="/dashboard" style={{ fontFamily: "'Unbounded',sans-serif", fontSize: ".85rem", fontWeight: 700, color: "var(--text)", textDecoration: "none" }}>PRIMIO</Link>
            <span style={{ padding: "2px 9px", borderRadius: 100, background: "var(--surface-2)", border: "1px solid var(--border)", fontSize: ".66rem", fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--muted)" }}>{t.badge}</span>
          </div>
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 9 }}>
            <div style={{ padding: "7px 13px", borderRadius: 10, background: "var(--surface-2)", border: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "36px 26px 60px", animation: "fade .35s ease both" }}>
        <Link href="/dashboard" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--muted)", fontSize: ".82rem", textDecoration: "none", marginBottom: 20 }}>{t.backDash}</Link>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 300px", gap: 16, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ ...card, padding: 22 }}>
              ...
              <div style={{ fontSize: ".74rem", color: "var(--muted)", marginTop: 6 }}>{title.length}/60</div>
            </div>
            ...
            <div style={{ ...card, padding: 22 }}>
              ...
              <div style={{ fontSize: ".74rem", color: "var(--muted)", marginTop: 6 }}>{t.urlHint}</div>
            </div>
            ...
            <div style={{ ...card, padding: 22 }}>
              ...
              />
              <div style={{ fontSize: ".74rem", color: "var(--muted)", marginTop: 6 }}>{description.length}/200</div>
            </div>
              ...
              <div style={{ marginBottom: 12 }}>
                {(newImagePreview || existingImageURL) && (
                  <div style={{ marginBottom: 12, display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    {!newImagePreview && (
                      <span style={{ fontSize: ".78rem", color: "var(--muted)" }}>{t.existingImage}</span>
                    )}
                  </div>
                )}
                <input
                ...
                <button
                  ...
                  style={{ padding: "10px 18px", borderRadius: 10, border: "1px dashed var(--border)", background: "var(--surface-2)", color: "#A78BFA", fontSize: ".82rem", cursor: "pointer" }}
                >
                  {newImageFile ? t.changeImage : t.updateImage}
                </button>
                <div style={{ fontSize: ".74rem", color: "var(--muted)", marginTop: 6 }}>{t.imageHint}</div>
              </div>
            </div>
            ...
            <div style={{ ...card, padding: 22 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 8 }}>
                {CAT_KEYS.map((k) => {
                  ...
                  const on = category === k;
                  return (
                    <button key={k} onClick={() => setCategory(k)} style={{ display: "flex", alignItems: "center", gap: 9, padding: "10px 14px", borderRadius: 11, border: `1px solid ${on ? "#7C3AED" : "var(--border)"}`, background: on ? "rgba(124,58,237,.12)" : "var(--surface-2)", cursor: "pointer", textAlign: "left" }}>
                      <span style={{ fontSize: "1.1rem" }}>{c.emoji}</span>
                      <div>
                        <div style={{ fontSize: ".8rem", fontWeight: 600, color: on ? "#A855F7" : "var(--text)" }}>{c.label}</div>
                      </div>
            ...
            <div style={{ ...card, padding: 22 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ color: "var(--muted)", fontFamily: "'JetBrains Mono',monospace", fontSize: "1.1rem" }}>$</span>
                <input
                  type="number"
                  min={1}
                  step={1}
                ...
                />
                <span style={{ fontSize: ".8rem", color: "var(--muted)" }}>{t.perDay}</span>
              </div>
              <div style={{ fontSize: ".74rem", color: "var(--muted)", marginTop: 6 }}>{t.bidHint}</div>
            </div>
            ...
            <div style={{ ...card, padding: 22 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: isCustomDur ? 10 : 0 }}>
                {DURATIONS.map((d) => {
                  ...
                  return (
                    <button key={d} onClick={() => { setIsCustomDur(false); setDuration(d); }} style={{ padding: "10px 0", borderRadius: 10, border: `1px solid ${on ? "#7C3AED" : "var(--border)"}`, background: on ? "rgba(124,58,237,.12)" : "var(--surface-2)", color: on ? "#A855F7" : "var(--text)", fontSize: ".88rem", fontWeight: on ? 700 : 500, cursor: "pointer" }}>
                      {d} {t.dayLabel}
                    </button>
                  );
                })}
                <button onClick={() => setIsCustomDur(!isCustomDur)} style={{ padding: "10px 0", borderRadius: 10, border: `1px solid ${isCustomDur ? "#7C3AED" : "var(--border)"}`, background: isCustomDur ? "rgba(124,58,237,.12)" : "var(--surface-2)", color: isCustomDur ? "#A855F7" : "var(--text)", fontSize: ".88rem", fontWeight: isCustomDur ? 700 : 500, cursor: "pointer" }}>
                  {t.customDur}
                </button>
              </div>
              {isCustomDur && (
                <div style={{ marginTop: 4 }}>
                  ...
                  />
                  <div style={{ fontSize: ".74rem", color: "var(--muted)", marginTop: 6 }}>{t.daysRange}</div>
                </div>
              )}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ ...card, padding: 18 }}>
              <div style={{ fontSize: ".68rem", letterSpacing: ".12em", textTransform: "uppercase", color: "var(--muted)", fontWeight: 700, marginBottom: 14 }}>{t.priceSummary}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: ".85rem" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", color: "var(--text)", fontWeight: 600 }}>
                    {isCustomDur ? (parseInt(customDays) || "–") : duration} {t.dayLabel}
                  </span>
                </div>
                <div style={{ height: 1, background: "var(--border)", margin: "4px 0" }} />
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "var(--text)", fontWeight: 700 }}>{t.total}</span>
            ...
            <div style={{ background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 16, padding: 18 }}>
              <div style={{ fontSize: ".68rem", letterSpacing: ".12em", textTransform: "uppercase", color: "var(--muted)", fontWeight: 700, marginBottom: 12 }}>{t.notes}</div>
