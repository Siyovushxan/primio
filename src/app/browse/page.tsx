...
import { rankAds } from "@/lib/adRules";
...
function AdCard({ ad, position, t, isOwn }: { ad: Ad; position: number; t: Translations; isOwn: boolean }) {
  const impressionRef = useImpression<HTMLDivElement>(ad.id, isOwn);
  ...
  return (
    <div ref={impressionRef} style={{
      background: `linear-gradient(160deg, rgba(42,42,53,.7) 0%, rgba(11,11,15,.97) 100%)`,
      border: `1px solid ${isTop3 ? "rgba(124,58,237,.4)" : "rgba(42,42,53,.6)"}`,
      borderRadius: 20,
      overflow: "hidden",
    ...
    onMouseLeave={e => {
      ...
      (e.currentTarget as HTMLElement).style.borderColor = isTop3 ? "rgba(124,58,237,.4)" : "rgba(42,42,53,.6)";
    }}>
      {/* Image */}
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ marginBottom: 8 }}>
          <h3 style={{ fontSize: ".95rem", fontWeight: 700, color: "var(--text)", marginBottom: 3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {ad.title}
          </h3>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 12, flexWrap: "wrap" }}>
          {/* Rank */}
          <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 8, background: "var(--surface-2)", border: "1px solid var(--border)" }}>
          ...
          {/* CTR */}
          <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "5px 10px", borderRadius: 8, background: "var(--surface-2)", border: "1px solid var(--border)" }}>
            <span style={{ fontSize: ".66rem", color: "var(--muted)" }}>CTR</span>
        ...
        <a
          ...
          onClick={() => { if (!isOwn) trackClick(ad.id); }}
...
export default function BrowsePage() {
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 64px)", background: "var(--bg)", color: "var(--text)", fontFamily: "system-ui,sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div className="browse-container" style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 24px 80px", position: "relative", zIndex: 1 }}>
        {/* ── Header ─────────────────────────────────────────────────────────── */}
        <div style={{ marginBottom: 20, animation: "fade .3s ease both" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6, flexWrap: "wrap" }}>
            ...
            {!loading && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: ".72rem", color: "#22C55E", fontFamily: "'JetBrains Mono',monospace", background: "rgba(34,197,94,.08)", border: "1px solid rgba(34,197,94,.25)", borderRadius: 100, padding: "4px 12px", whiteSpace: "nowrap", animation: "badge-pulse 3s ease-in-out infinite" }}>
          ...
          {/* Row 2: subtitle */}
          <p style={{ fontSize: ".86rem", color: "var(--muted)", lineHeight: 1.55, marginBottom: 16 }}>{t.browseSub}</p>

          {/* Row 3: stats — compact horizontal chips */}
          {!loading && (() => {
            ...
            return (
              <div className="stats-scroll" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {[
                  ...
                  { label: t.browseStats[2], value: fmt(totalClk), color: "#FCD34D" },
                  { label: t.browseStats[3], value: ctr, color: "#F59E0B" },
                ].map((s) => (
                  <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 14px", borderRadius: 10, background: "rgba(21,21,27,.8)", border: "1px solid rgba(42,42,53,.9)" }}>
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: ".88rem", fontWeight: 700, color: s.color }}>{s.value}</span>
                    <span style={{ fontSize: ".72rem", color: "var(--dim)", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".08em", whiteSpace: "nowrap" }}>{s.label}</span>
                  </div>
                ))}
              </div>
            );
        ...
        {/* ── Divider ─────────────────────────────────────────────────────────── */}
        <div style={{ height: 1, background: "linear-gradient(90deg,rgba(124,58,237,.25),rgba(42,42,53,.5),transparent)", marginBottom: 18 }} />
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div className="cats-list" style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            <button
              ...
              style={{ padding: "8px 18px", borderRadius: 100, border: `1px solid ${!activeCat ? "#7C3AED" : "var(--border)"}`, background: !activeCat ? "#7C3AED" : "var(--surface-2)", color: !activeCat ? "#fff" : "var(--muted)", fontSize: ".82rem", fontWeight: 700, cursor: "pointer" }}
            >
              {t.browseAll}
            </button>
            {CATEGORY_KEYS.map((cat) => {
              ...
              return (
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 100, border: `1px solid ${active ? "#7C3AED" : "var(--border)"}`, background: active ? "#7C3AED" : "var(--surface-2)", color: active ? "#fff" : "var(--muted)", fontSize: ".82rem", fontWeight: 600, cursor: "pointer", transition: "all .15s" }}
                >
        ...
        {loading ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 20 }}>
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} style={{ background: "linear-gradient(90deg,var(--surface) 25%,var(--surface-3) 50%,var(--surface) 75%)", backgroundSize: "200% 100%", borderRadius: 20, height: 320, animation: `shimmer 1.6s ease infinite ${i * .1}s` }} />
            ))}
          </div>
        ) : ads.length === 0 ? (
          <div style={{ background: "linear-gradient(135deg, rgba(42,42,53,.5) 0%, rgba(21,21,27,.8) 100%)", border: "1px solid rgba(124,58,237,.2)", borderRadius: 24, padding: "64px 32px", textAlign: "center" }}>
            <div style={{ fontSize: "3.5rem", marginBottom: 16 }}>
              {activeCat ? CATEGORIES[activeCat].emoji : "📢"}
            </div>
            <h2 style={{ fontSize: "1.2rem", fontWeight: 700, color: "var(--text)", marginBottom: 8 }}>{t.browseEmpty}</h2>
            <p style={{ color: "var(--muted)", marginBottom: 24, fontSize: ".9rem" }}>{t.browseEmptySub}</p>
            <Link
              href={activeCat ? `/create?cat=${activeCat}` : "/create"}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(290px,1fr))", gap: 20 }}>
            {ads.map((ad, i) => (
              <div key={ad.id} style={{ animation: `fade .35s ease ${i * .05}s both` }}>
                <AdCard ad={ad} position={i + 1} t={t} isOwn={ad.advertiserUID === firebaseUser?.uid} />
              </div>
            ))}
          </div>
        )}
