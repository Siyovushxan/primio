    ...
    <div style={{ minHeight: "calc(100vh - 64px)", background: "#0A0715", color: "#EDE9FE", fontFamily: "system-ui,sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div className="browse-container" style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 24px 80px", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ marginBottom: 20, animation: "fade .3s ease both" }}>
          ...
          {/* Row 2: subtitle */}
          <p style={{ fontSize: ".86rem", color: "#6D5B8E", lineHeight: 1.55, marginBottom: 16 }}>{t.browseSub}</p>
            ...
            return (
              <div className="stats-scroll" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {[
                  ...
                  <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 14px", borderRadius: 10, background: "rgba(22,15,42,.8)", border: "1px solid rgba(45,31,80,.9)" }}>
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: ".88rem", fontWeight: 700, color: s.color }}>{s.value}</span>
                    <span style={{ fontSize: ".72rem", color: "#4A3C6E", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".08em", whiteSpace: "nowrap" }}>{s.label}</span>
                  </div>
                ))}
              </div>
            );
        ...
        {/* ── Divider ─────────────────────────────────────────────────────────── */}
        <div style={{ height: 1, background: "linear-gradient(90deg,rgba(124,58,237,.25),rgba(45,31,80,.5),transparent)", marginBottom: 18 }} />
