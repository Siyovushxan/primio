...
export default function Header() {
  ...
  return (
    <>
      <header style={{ position: "sticky", top: 0, zIndex: 80, background: "rgba(11,11,15,.92)", backdropFilter: "blur(18px)", borderBottom: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "12px 20px", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <Link href="/" style={{ display: "flex", alignItems: "center", gap: 9, textDecoration: "none", flexShrink: 0 }}>
            ...
            <span style={{ fontFamily: "'Unbounded', sans-serif", fontSize: ".9rem", fontWeight: 700, letterSpacing: ".01em", color: "var(--text)" }}>PRIMIO</span>
          </Link>
          ...
          <nav className="hidden md:flex" style={{ gap: 2, marginLeft: 8 }}>
            {NAV_ITEMS.map((n) => (
              <Link key={n.href} href={n.href}
                style={{ padding: "8px 13px", borderRadius: 9, color: "var(--muted)", fontSize: ".85rem", fontWeight: 600, textDecoration: "none", transition: "color .15s" }}
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div className="hidden md:flex" style={{ alignItems: "center", gap: 2, padding: "4px 6px", borderRadius: 9, background: "var(--surface-2)", border: "1px solid var(--border)" }}>
              {LANGS.map((l) => (
                <button key={l.code} onClick={() => setLang(l.code)} style={{ padding: "4px 9px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: ".73rem", fontWeight: 700, letterSpacing: ".04em", background: lang === l.code ? "#7C3AED" : "transparent", color: lang === l.code ? "#fff" : "var(--muted)", transition: "all .15s" }}>
                  {l.label}
                </button>
              ))}
            </div>
              ...
              <>
                {firebaseUser ? (
                  <>
                    ...
                    <div className="hidden md:flex" style={{ alignItems: "center", gap: 8, padding: "7px 13px", borderRadius: 10, background: "var(--surface-2)", border: "1px solid var(--border)" }}>
                      <span style={{ fontSize: ".68rem", letterSpacing: ".1em", textTransform: "uppercase", color: "var(--muted)", fontWeight: 700 }}>{t.navSpent}</span>
                    ...
                    {/* Avatar — desktop only */}
                    <Link href="/dashboard" className="hidden md:flex" style={{ alignItems: "center", gap: 8, padding: "5px 11px 5px 5px", borderRadius: 100, background: "var(--surface-2)", border: "1px solid var(--border)", color: "var(--text)", textDecoration: "none" }}>
            ...
            {/* display comes from the classes: an inline display would override md:hidden */}
            <button
              className="flex md:hidden"
              onClick={() => setDrawerOpen(true)}
              style={{ padding: "8px", color: "#A78BFA", background: "rgba(124,58,237,.12)", border: "1px solid rgba(124,58,237,.25)", borderRadius: 9, alignItems: "center", justifyContent: "center" }}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          <button onClick={close} style={{ padding: 7, color: "var(--muted)", background: "rgba(163,161,180,.12)", border: "1px solid var(--border)", borderRadius: 8, display: "flex" }}>
            <X size={18} />
          </button>
        </div>
        ...
        {!loading && firebaseUser && (
          <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: ".84rem", fontWeight: 700, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{userProfile?.displayName}</div>
        ...
        <nav style={{ flex: 1, padding: "12px 12px" }}>
          <div style={{ fontSize: ".63rem", letterSpacing: ".12em", textTransform: "uppercase", color: "var(--dim)", fontWeight: 700, padding: "8px 10px 6px" }}>Menyu</div>
        ...
        <div style={{ padding: "14px 20px", borderTop: "1px solid var(--border)" }}>
          <div style={{ fontSize: ".63rem", letterSpacing: ".12em", textTransform: "uppercase", color: "var(--dim)", fontWeight: 700, marginBottom: 10 }}>Til</div>
          <div style={{ display: "flex", gap: 6 }}>
            {LANGS.map((l) => (
              <button key={l.code} onClick={() => { setLang(l.code); close(); }}
                style={{ flex: 1, padding: "9px 0", borderRadius: 9, border: `1px solid ${lang === l.code ? "#7C3AED" : "var(--border)"}`, cursor: "pointer", fontSize: ".8rem", fontWeight: 700, letterSpacing: ".05em", background: lang === l.code ? "rgba(124,58,237,.2)" : "transparent", color: lang === l.code ? "#A855F7" : "var(--muted)", transition: "all .15s" }}>
                {l.label}
              </button>
            ))}
          </div>
